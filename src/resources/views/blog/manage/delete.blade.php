                    ...
                    @endif
                        ...
                        <div class="list-item">
                            <div class="btn-group pull-right">
                                ...
                                @else
                                <a href="{{url('blog/manage/delete')}}?p={{$c}}" class="btn btn-default btn-sm">首页</a>
                                @endif
                                
                                @if($p - 1 <= 0)
                                <a href="jacascript:void(0)" class="btn btn-default btn-sm disabled">上一页</a>
                                @else
                                <a href="{{url('blog/manage/delete')}}?p={{$p-1}}" class="btn btn-default btn-sm">上一页</a>
                                @endif

                                @for($i = -3; $i <= 3; $i++)
                                @if($p + $i > 0 && $p + $i <= $c)
                                @if($i != 0)
                                <a href="{{url('blog/manage/delete')}}?p={{$p+$i}}" class="btn btn-default btn-sm">{{$p + $i}}</a>
                                @else
                                <a href="{{url('blog/manage/delete')}}?p={{$p+$i}}" class="btn btn-default btn-sm disabled">{{$p + $i}}</a>
                                @endif
                                @endif
                                @endfor
                                ...
                                @else
                                <a href="{{url('blog/manage/delete')}}?p={{$p+1}}" class="btn btn-default btn-sm">下一页</a>
                                @endif

                                @if($c <= 0)
                                <a href="jacascript:void(0)" class="btn btn-default btn-sm disabled">尾页</a>
                                @else
                                <a href="{{url('blog/manage/delete')}}?p={{$c}}" class="btn btn-default btn-sm">尾页</a>
                                @endif
                            </div>
                        </div>
