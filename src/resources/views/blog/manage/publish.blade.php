            ...
            <div class="mb-block write">
                <div class="mb-block-content">
                        ...
                        <div class="list-item">
                            <div class="item-title">
                                ...
                                <div class="item-bottom  col-md-12 col-xs-12 col-sm-12">
                                    <a href="{{url('blog')}}?id={{$blog->blog_id}}">查看({{$blog->blog_view_count}})</a>
                                    <a href="{{url('blog')}}?id={{$blog->blog_id}}#comment">评论({{$blog->comment_count}})</a>
                                    <a href="{{url('blog/undo')}}?id={{$blog->blog_id}}" class="item-btn btn btn-xs btn-warning pull-right">撤回</a>
                    ...
                    @endif
                        ...
                        <div class="list-item">
                            <div class="pull-left">
                                <button class="btn btn-warning btn-sm">批量撤回</button>
                            ...
                            <div class="btn-group pull-right">
                                ...
                                @else
                                <a href="{{url('blog/manage/publish')}}?p={{$c}}" class="btn btn-default btn-sm">首页</a>
                                @endif
                                
                                @if($p - 1 <= 0)
                                <a href="jacascript:void(0)" class="btn btn-default btn-sm disabled">上一页</a>
                                @else
                                <a href="{{url('blog/manage/publish')}}?p={{$p-1}}" class="btn btn-default btn-sm">上一页</a>
                                @endif

                                @for($i = -3; $i <= 3; $i++)
                                @if($p + $i > 0 && $p + $i <= $c)
                                @if($i != 0)
                                <a href="{{url('blog/manage/publish')}}?p={{$p+$i}}" class="btn btn-default btn-sm">{{$p + $i}}</a>
                                @else
                                <a href="{{url('blog/manage/publish')}}?p={{$p+$i}}" class="btn btn-default btn-sm disabled">{{$p + $i}}</a>
                                @endif
                                @endif
                                @endfor
                                ...
                                @else
                                <a href="{{url('blog/manage/publish')}}?p={{$p+1}}" class="btn btn-default btn-sm">下一页</a>
                                @endif

                                @if($c <= 0)
                                <a href="jacascript:void(0)" class="btn btn-default btn-sm disabled">尾页</a>
                                @else
                                <a href="{{url('blog/manage/publish')}}?p={{$c}}" class="btn btn-default btn-sm">尾页</a>
                                @endif
                            </div>
                        </div>
